...
| 0.4 | LLMClient | Tier→provider binding ([ADR-003](../ADRs.md#adr-003)); vendor SDKs lazy so it imports offline | `src/fieldops/llm/{base,openai_client,grok_client,factory}.py` | imports without keys; `llm-health` probes set keys | `feat(llm): provider-agnostic client (OpenAI + Grok)` | ✅ |
