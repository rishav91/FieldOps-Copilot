...
| **Classifier + routing** | Agency + complaint-type label with **calibrated** confidence. | Cascade: embedding/cheap model → Grok (cheap tier) → (rare) OpenAI. See [AI-ARCH §3](AI-ARCHITECTURE.md#3-the-classification-cascade-funnel). | Calibrated via Platt/isotonic on a held-out set ([ADR-007](ADRs.md#adr-007)). |
